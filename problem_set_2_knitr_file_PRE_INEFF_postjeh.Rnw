...
while (((happy_counter/n) < stop.val) & (cycles < cyclemax)){
     ...
     ##For a point in the location table...
       ...
       neighborracevector <- matrix(, ncol = testpool, nrow = 1 )
       
       for (neighbor in (1:testpool)){
       ## For each closest neighbor of the given point
         
         neighborList <- neighborList[,1:testpool]
         ...
         a_neighbors_race <- LocationTable[a_neighbor,2]
         ## Find neighbor's race
        
         if (own_race == a_neighbors_race){
      ...
      totalownraceneighbors = sum(neighborracevector[1,])
      
      
      
      if (totalownraceneighbors >= (threshold)){
           ...
           xstar <- runif(1, min=0, max=1)
           ystar <- runif(1, min=0, max=1)
        
           LocationTable[individual,3] <- xstar
           LocationTable[individual,4] <- ystar
